...
namespace PipelinedOrchestrationService
{
    ...
    /// to service queued work on the thread pool. Each work cycle handles up to <see cref="MAXBATCHSIZE"/> elements. 
    ...
    /// </summary>
    abstract class BatchWorker<T> where T : class
    {
        ...
        volatile TaskCompletionSource<int>? shutdownCompletionSource;
        ...
        protected abstract ValueTask Process(IList<T> batch);
        ...
        public Task WaitForShutdownAsync()
        {
            ...
            {
                Interlocked.CompareExchange(ref this.shutdownCompletionSource, new TaskCompletionSource<int>(), null);
                this.NotifyInternal();
            }
        ...
        async ValueTask Work()
        {
            ...
            if (this.cancellationToken.IsCancellationRequested)
            {
                this.shutdownCompletionSource?.TrySetResult(0);
            }
        }
